...
import { roadFurniture } from '../datasheetTemplates/otherTemplates';

import UnderscoreFormatter from '../helpers/underscoreFormatter';
import ZeroChecker from '../helpers/zeroCheckRoad';

import {Toast} from 'native-base';


const UploadRoadFurniture = (props) => {    
    const [road, setRoad] = useState(roadFurniture);
 
    const [title, setTitle] = useState("Road Furniture");
    const [location, setLocation] = useState({});
    const [type, setType] = useState("road_furniture");
    ...
    const handleBreakdown = (indy, valz) => {
      let dup_array = road.road==undefined?road:road.road
      dup_array[indy].breakdown = valz
       setRoad(prevState=>({
           road:dup_array
       }))
    }
    ...
    const handleLength = (indy, valz) => {
      let dup_array = road.road==undefined?road:road.road
      dup_array[indy].length_installed = valz.replace(/[^0-9]/g, '')
       setRoad(prevState=>({
           road:dup_array
       }))
    }
...
return (
  ...
  <ScrollView showsVerticalScrollIndicator={false} style={{marginTop:50, marginBottom:20}}>
    ...
    <View style={[styles.eachCard]}>
       ...
       <Text style={styles.title}>{ UnderscoreFormatter(params.description) }</Text>
      ...
      <View style={{flexDirection:'column', width:'50%'}}>
          ...
          <TextInput
            ...
            onChangeText={text => handleBreakdown(index, text)}
        ...
        <View style={{flexDirection:'column', width:'40%'}}>
          ...
          <TextInput
            ...
            placeholder="length installed"
            textAlign={'center'}
            style={{ borderRadius:5, height: 40, width:'100%', fontFamily:'Montserrat_400Regular', borderColor: 'gray', borderWidth: 1 }}
            onChangeText={text => handleLength(index, text)}
            value={params.length_installed}
            />
        </View>
...
export default UploadRoadFurniture;
